...
当前摘要（2026-09-10 15:38）：用户已解锁并要求继续，锁屏暂停已解除。已实操检查本地登录、关注预览取消、创作者列表、复盘固定与日历描述、个人订阅复制反馈和ICS下载。仅修复公共订阅未开放时无效重试的页面提示，引导个人订阅；普通故障仍可重试，原SQL公共订阅仍可用。构建/类型与格式检查通过，1440/1280/1024订阅页无横向溢出。下载文件12事件/12唯一UID，含合成复盘链接，仅为本地证据。前端预览3002已更新，session59568/PID13957；3008后端未重启。只在3008固定了9月8日既有复盘，关注预览未保存。无push或部署，不扩展基础设施。详情见客户端output/playwright/lean-check-2026-09-10.md。

以下为此前记录；其中锁屏暂停与页面待解锁状态已被本轮证据取代。
