...
完整验收边界见 [STATE.md](STATE.md)、[当前架构](docs/architecture.md) 与 [验收证据](evidence/local-2026-09-09.md)。Codex CLI 0.153.4 的本地 OAuth、工具发现和撤销已实测；实际工具调用、系统日历刷新、Azure 触发器和手机内容直达仍待验收。独立Firebase开发项目已完成真实Google登录、服务端验签、偏好持久化和退出验证，见 [开发云环境](docs/cloud-development.md)。可运行 `uv run python -m scripts.check_codex_discovery` 检查已授权的本机 Codex 连接，操作与清理步骤见 [MCP 文档](docs/mcp-and-connections.md)。
