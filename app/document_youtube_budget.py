...
class Budget:
    ...
    def prepared(self, old, instant):
        ...
        if value["period"] < period:
            value.update(period=period, reserved_units=0, daily_limit=self.cfg.youtube_daily_budget)
        else:
    ...
    def reserve(self, endpoint):
        ...
        for _ in range(32):
            ...
            if error:
                raise error
            return Reservation(self.cfg.youtube_project_id, period, reset)
    ...
    def upstream_wait(self, ticket, code, seconds=60):
        ...
        for _ in range(32):
            ...
            value = self.prepared(old, instant)
            if value["blocked_until"] and datetime.fromisoformat(value["blocked_until"]) > resume:
            ...
            try:
                ...
                continue
            return wait_error(value["reason"], resume, instant)
        raise StoreError("YOUTUBE_BUDGET_BUSY", retryable=True, retry_after=1)
