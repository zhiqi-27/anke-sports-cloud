...
2026-09-10。Anke Sports 的初始云目标继续为 Cosmos NoSQL **Serverless + Periodic**；以后按实测负载原地转手动 Provisioned，再调整 Autoscale。Anke Money 当前生产组合以用户本次说明为准，没有读取或修改其资源。
...
尚未接入文档模式：体育 Provider 抓取/定时更新、个人视频链接和屏蔽、创作者/YouTube、直播、公共 Feed、单场选择写入、配置导入、账号删除、OAuth/MCP、Google 直连。相关接口明确返回 `DOCUMENT_FEATURE_UNAVAILABLE`。包含尚未迁移内容配置的账号不能被静默发布为空链接日历。设置页的 Provider/YouTube 不代表后台已启用。
...
```sh
uv run python -m experiments.document_ui
```

打开 [日历](http://localhost:3006/calendar) 或 [我的关注](http://localhost:3006/following)，进入本地体验，选择演示联赛、预览、保存。12条演示比赛包含历史场次。该实验的持久文档/本地队列都使用独立 SQLite 文件，只是显式本地适配器，**不是 Cosmos 或 Azure Queue 模拟器**。不会读写原用户数据库或调用平台。
...
真实 Cosmos 身份权限、RU/429、分区拆分和吞吐、Azure Queue/poison/Timer、Periodic 恢复、删除决定重放、SQL 数据迁移仍未通过。大配置/回执分块、历史任务/孤立块 GC、超大 Feed 流式读取与反向索引仍需实现。容量转换保持独立管理流程，参见 [存储与扩容方案](cosmos-storage-design.md)。
