...
> 当前 A 候选（2026-09-17）：本文早期创作者/视频段落均为历史记录。当前运行时不加载这些模块，不保留旧兼容层；手动来源 schema/OpenAPI 已落地，单场增删仍属于后续 B/C。
...
已接入个人原链接附加、屏蔽/固定、配置导入预览与确认。旧事件加入/排除/重置和创作者导入不属于当前入口；当前手动单场来源仅进入 schema/OpenAPI，增删由后续 B/C 实现。链接、配置、新任务和精确幂等回执在个人分区同事务提交；屏蔽后重新附加不会撤销屏蔽。共享链接筛选和导入规则由 SQL 与文档模式共用，合并超出配置上限返回明确错误。

早期文档模式曾接入 YouTube 额度、频道、匹配和人工确认；这些模块、配置和任务现仅作历史追溯，当前运行时不加载，也不进入状态或发布接口。

尚未接入或仍待验收的现行能力以工作区 STATE 和实施计划为准；旧创作者/WebSub接口不再作为兼容 fallback。未迁移接口明确返回 `DOCUMENT_FEATURE_UNAVAILABLE`。

完整客户端合同仍从 SQL 基线导出，但本轮已重新生成并包含 `manual_events` 与私有事件来源字段；文档模式不另行维护公开合同。新路径不是完整 Cosmos 迁移，也不是可直接公测的部署包。
