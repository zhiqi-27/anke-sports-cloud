...
先按个人赛事和作者范围限定候选，再检查双方球队/大奖赛分场次、前瞻/复盘词、显式日期、发布时间窗口与排除词。当前 `matching-v2` 要求标题自身提供自动附加所需的对象、阶段与日期证据；简介只能补充候选，不能将旧推广误当本视频主题。比分和有多种解释的日期不用于自动消歧。前瞻窗口 7 天、复盘窗口 3 天；相同对手多场、F1 未指明分场次或类型不清时进入待确认。规则版本和原因代码随结果保存，不显示概率或未经标注集验证的准确率。离线差异与边界见 [规则回放](matching-replay.md)。

标题改变导致矛盾时撤下自动链接。确认会固定链接；忽略/移除会写入个人 block，后台不会加回。暂停保留旧关联，删除作者前预览影响；手动/确认/固定的链接保留。公共比赛 ID、个人 Feed UID 不因内容变化或令牌轮换而变化。

现已缩小视频时间/旧关联候选及个人Feed关注候选，并合并同账号待开始的发布任务。20k赛事、1,000账号、200创作者的单通知本机共享发布见 [多人容量与边界](content-capacity.md)。真实并发云环境、200条真实标注视频及98%精确率仍未验收；项目级YouTube预算的本地实现见 [配额说明](youtube-budget.md)。
