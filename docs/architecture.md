...
- 私人链接和公共来源分离，用户 block 覆盖自动发现。当前自动发现尚未实现；手工添加不获得官方认证。
...
需独立 Anke Sports Firebase 项目、Azure 订阅/资源组/区域、Web 与 API HTTPS 地址、MySQL 与 Storage。部署前明确目标和费用边界，验证 MySQL 备份恢复与迁移回滚；无需改动现有 FormaLM。
